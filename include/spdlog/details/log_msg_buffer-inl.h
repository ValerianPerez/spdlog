...
SPDLOG_INLINE log_msg_buffer::log_msg_buffer(const log_msg &orig_msg)
    ...
    buffer.append(reinterpret_cast<const char *>(field_data),
                  reinterpret_cast<const char *>(field_data + field_data_count));
...
SPDLOG_INLINE log_msg_buffer::log_msg_buffer(log_msg_buffer &&other) SPDLOG_NOEXCEPT
    ...
    std::cout << "this_count=" << field_data_count << " other_count=" << other.field_data_count
              << std::endl;
    update_string_views();
}
...
SPDLOG_INLINE void log_msg_buffer::update_string_views() {
    ...
    for (size_t i = 0; i < field_data_count; i++) {
        ...
        if (string_view_p) {
            field_data[i].value =
                FieldValue(string_view_t{buffer.data() + offset, string_view_p->size()});
